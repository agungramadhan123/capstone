...
def clean_data_yaml(yaml_path: Path, dry_run: bool = False) -> dict:
    ...
    if not yaml_path.exists():
        print(f"  File tidak ditemukan: {yaml_path}")
        return {}
    ...
    if new_names != VALID_CLASSES:
        print(f"  PERINGATAN: Kelas tidak sesuai ekspektasi!")
...
def remap_labels(label_dir: Path, index_map: dict, dry_run: bool = False):
    ...
    if not label_dir.exists():
        print(f"  Direktori label tidak ditemukan: {label_dir}")
        return

    label_files = sorted(label_dir.glob("*.txt"))
    if not label_files:
        print(f"  Tidak ada file label di: {label_dir}")
        return
...
def validate_model(data_yaml_path: Path, model_path: Path):
    ...
    try:
        ...
        # Tampilkan hasil
        print("\n  HASIL VALIDASI ULANG (CLEAN DATASET)")
...
def main():
    ...
    if not args.skip_validate and not args.dry_run:
        ...
        if val_yaml:
            ...
            print("  Tidak ada data.yaml untuk validasi.")
    elif args.skip_validate:
        print("\n  Validasi dilewati (--skip-validate)")
    elif args.dry_run:
        print("\n  Validasi dilewati (--dry-run mode)")
