...
PROJECT_ROOT = Path(__file__).resolve().parent.parent  # Naik ke root project

MODEL_PRETRAINED = str(PROJECT_ROOT / "models" / "best.pt")
DATA_YAML        = str(PROJECT_ROOT / "config" / "data.yaml")
HYP_YAML         = str(PROJECT_ROOT / "config" / "train_cctv.yaml")
PROJECT_NAME     = "cctv_bubat"
RUN_NAME         = "finetune_v1"

def _find_data_yaml(path):
    if os.path.exists(path):
        return path
    matches = glob.glob(str(PROJECT_ROOT / "**" / "data.yaml"), recursive=True)
...
if not os.path.exists(MODEL_PRETRAINED):
    MODEL_PRETRAINED = str(PROJECT_ROOT / "models" / "yolov8s.pt")
